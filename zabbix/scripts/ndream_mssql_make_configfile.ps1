...
{
    if ($name.name -eq "master" -or $name.name -eq "tempdb" -or $name.name -eq "model" -or $name.name -eq "msdb")
        {
            $idx++;
            continue
        }
    if ($idx -lt $basename.Rows.Count)
        {
            $line= "{ `"{#INST}`" : `"" + $name.inst + "`", "  + "`"{#DBNAME}`" : `"" + $name.name + "`", `"{#DBLOCK}`" : `"_Total`" }," | convertto-encoding "cp866" "utf-8"
            write-host $line
        }
